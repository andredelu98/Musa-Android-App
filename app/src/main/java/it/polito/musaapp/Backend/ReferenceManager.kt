...
fun InsertImageInSaved(s: String, vm: MusaViewModel){
    if(vm.savedRef.value.isNullOrEmpty()){
        ...
        Firebase.database.getReference("ReferenceSalvate").child("Ref${vm.referenceCounter.value}")
            .setValue(s)
    }
...
fun RemoveImageInSaved(s: String, vm: MusaViewModel){
    ...
    if(vm.getRefToRemove(s)!=-1)
        Firebase.database.getReference("ReferenceSalvate").child("Ref${vm.getRefToRemove(s)}").removeValue()
